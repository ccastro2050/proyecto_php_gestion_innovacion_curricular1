...
`RepositorioAliadoMariaDB` encapsula la conexión PDO y el SQL: sus
...
`ServicioAliado` no HEREDA de un repositorio, RECIBE un repositorio por
...
`implements IRepositorioAliado` puede ocupar el lugar de otra — el MariaDB
...
classDiagram
    ...
    RepositorioAliadoMariaDB ..|> IRepositorioAliado : POLIMORFISMO
    ...
    note for RepositorioAliadoMariaDB "ENCAPSULAMIENTO: la conexión y el SQL no salen de aquí"
```
...
classDiagram
    ...
    ServicioMal --|> RepositorioAliadoMariaDB : hereda del CONCRETO — quedó casado con MariaDB
...
sequenceDiagram
    ...
    participant C as ControladorAliado
    participant S as ServicioAliado
    participant R as IRepositorioAliado (interface)
...
- **Constructor promotion + `readonly`** (PHP 8): el constructor declara,
  ...
  `private readonly IRepositorioAliado $repositorio`.
- **El modelo es el dato como objeto**: la clase `Aliado` con sus 4
...
```
Aliado (el modelo)         ← la clase entidad: el dato con tipos
ControladorAliado          ← objeto HTTP; valida el body y compone un IServicioAliado
ServicioAliado             ← objeto de NEGOCIO; compone un IRepositorioAliado
IRepositorioAliado         ← contrato (interface): abstracción pura
RepositorioAliadoMariaDB   ← implementación concreta (encapsula PDO y SQL)
...
(`RepositorioAliadoPostgreSQL`) entrará por la misma puerta.
