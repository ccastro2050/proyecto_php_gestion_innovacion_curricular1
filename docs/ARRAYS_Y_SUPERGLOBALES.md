...
|---|---|---|
| **Lista** — llaves 0,1,2… | `List<T>`, `T[]` | La lista de aliados que devuelve `obtenerTodos()` |
| **Diccionario** — llaves de texto | `Dictionary<string,T>`, `Map` | `$datos` en el `UPDATE` dinámico: columna → valor |
| **Registro** — llaves fijas y conocidas | una `struct`, una clase | La fila que devuelve PDO: `['nit' => …, 'razon_social' => …]` |
...
$dicc   = ['nit' => 10101, 'activo' => true];

$lista[] = 'd';                        // agregar al final: la llave 3 sale sola
$dicc['razon_social'] = 'Algo';     // agregar o reemplazar por llave
```
...
$aliados[10101] = ['nit' => 10101, 'razon_social' => 'Primera'];
$aliados[10102] = ['nit' => 10102, 'razon_social' => 'Segunda'];

// ---------- READ: una sola, y todas ----------
echo $aliados[10101]['razon_social'], "
";        // Primera
echo $aliados[99999]['razon_social'] ?? 'no existe', "
";   // no existe

foreach ($aliados as $llave => $ficha) {
    echo $llave, ' -> ', $ficha['razon_social'], "
";
}

// ---------- UPDATE: cambiar un campo ----------
$aliados[10101]['razon_social'] = 'Corregida';
echo $aliados[10101]['razon_social'], "
";        // Corregida
...
- **El UPDATE de arriba SÍ funciona**, y es importante entender por qué:
  `$aliados[…]['razon_social'] = 'Corregida'` escribe **directo en el
  almacén**. Si en cambio hubiera hecho `$ficha = $aliados[…];` y luego
  `$ficha['razon_social'] = 'Corregida';`, **no habría pasado nada** —
...
**`api_innovacion/pruebas/prueba_capas.php`**. Ahí adentro hay una clase que cumple
...
```php
class RepositorioFalsoEnMemoria implements IRepositorioAliado
{
...
```php
public function crear(Aliado $entidad): bool
{
    $this->datos[$entidad->getNit()] = $entidad;
    return true;
}
```
...
```php
public function obtenerPorClave($clave): ?Aliado
{
...
public function actualizar($clave, array $datos): int
{
    ...
    if (array_key_exists('razon_social', $datos)) {
        $fila->setRazonSocial($datos['razon_social']);
    }
...
Ejemplo real, del controlador (`api_innovacion/controladores/ControladorAliado.php`):
...
$p->setRazonSocial('otro');
echo $o->getRazonSocial();   // 'otro'  ← sí se enteró
```
...
$fila->setRazonSocial(…);  // por eso esto SÍ modifica lo guardado
```
...
```json
{"razon_social": null}   ← lo mandó, y dice "sin valor"
...
<?php else: ?>                    <!-- no hay: "todavía no hay aliados" -->
```
...
objetos del modelo (`api_innovacion/repositorios/RepositorioAliadoMariaDB.php`):
...
$sql = 'UPDATE aliado SET ' . implode(', ', $asignaciones)
     . ' WHERE nit = :clave_de_la_fila';
```

Con `$datos = ['razon_social' => 'X', 'razon_social' => 9]` sale:

```sql
UPDATE aliado SET razon_social = :razon_social, razon_social = :razon_social WHERE nit = :clave_de_la_fila
```
...
(`api_innovacion/index.php`):

```php
$metodo = $_SERVER['REQUEST_METHOD'];                          // "GET", "POST", "PUT"…
$ruta   = parse_url($_SERVER['REQUEST_URI'], PHP_URL_PATH);    // "/api/aliado"
```
...
Si se comparara `$_SERVER['REQUEST_URI'] === '/api/aliado'`, la petición
`/api/aliado?limite=5` **no coincidiría con ninguna ruta** y daría 404.
...
|---|---|---|
| Inyección de SQL | Los valores **nunca** se pegan al SQL: van como parámetros de PDO | `api_innovacion/repositorios/RepositorioAliadoMariaDB.php` |
...
1. **PDO devuelve arreglos.** `fetchAll(PDO::FETCH_ASSOC)` entrega cada fila
   como `['nit' => …, 'razon_social' => …]`, con **todo como texto**
...
1. Abra `api_innovacion/pruebas/prueba_capas.php`, busque en el repositorio falso
...
| | |
|---|---|
| [3_plan.md](spec_kit/versiones/v1_aliado/3_plan.md) | Las capas de esta versión y por qué están cortadas ahí |
| [6_contracts.md](spec_kit/versiones/v1_aliado/6_contracts.md) | El sobre del JSON: qué array recibe el front en cada respuesta |
