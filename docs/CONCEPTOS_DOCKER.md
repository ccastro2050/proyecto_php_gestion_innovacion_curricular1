...
tiene 2: `./api_innovacion`, `./front_php`.
...
COPY . .
EXPOSE 8115
CMD ["php", "-S", "0.0.0.0:8115", "index.php"]
```
...
| `EXPOSE 8115` | **Documenta** en qué puerto escucha el programa | No abre nada: quien publica el puerto es el `ports:` del compose |
| `CMD ["php", "-S", "0.0.0.0:8115", "index.ph…` | **El comando que se ejecuta al encender** el contenedor | Si ese proceso termina, el contenedor se apaga |
...
- En la v1: `proyecto_php1-mariadb-1` es un contenedor creado desde la imagen
  `mariadb:11`, con el puerto interno 3306 **publicado** en el 13332 de su PC
  (`"13332:3306"` en el compose).
...
flowchart LR
    ...
    subgraph PC["Su PC — Docker Desktop (el 'centro de datos')"]
        subgraph RED["red interna del compose (LAN virtual, con DNS propio)"]
            FRONTPHP["SERVIDOR WEB (front)<br/>contenedor front-php<br/>hostname: front-php · escucha en 8114"]
            APIINNOVACION["SERVIDOR DE APLICACIONES<br/>contenedor api-innovacion<br/>hostname: api-innovacion · escucha en 8115"]
    ...
    NAV -->|"localhost:8115"| APIINNOVACION
    NAV -->|"localhost:8114"| FRONTPHP
    NAV -->|"localhost:8107"| PHPMYADMIN
    APIINNOVACION -->|"mariadb:3306 (DNS de Docker)"| MARIADB
    FRONTPHP -->|"http://api-innovacion:8115<br/>al controlador, por el NOMBRE"| APIINNOVACION
    PHPMYADMIN -->|"mariadb:3306 (DNS de Docker)"| MARIADB
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:13332"| MARIADB
```
...
services:                          # el mapa de TODOS los contenedores del sistema
  ...
  mariadb:                         # ← este nombre es también su HOSTNAME interno
    ...
    environment:                   # variables que la imagen usa al crear la BD
      ...
      MARIADB_DATABASE: innovacion_php_local
    ...
    ports:
      - "13332:3306"               # "puerto en su PC : puerto interno del contenedor"
  ...
  api-registros:
    ...
    volumes:
      - ./api_innovacion:/app        # el código montado: guardar un .php = refrescar
    restart: unless-stopped        # si el proceso muere, Docker lo levanta de nuevo
    ports:
      - "8115:8115"                # http://localhost:8115
    environment:
      ...
      DB_DSN: mysql:host=mariadb;port=3306;dbname=innovacion_php_local
...
1. **Dos redes de nombres**: hacia su PC, puertos publicados
   (`localhost:8115`, `localhost:8101`, `localhost:13332`); entre
...
docker network create proyecto_php_gestion_innovacion_curricular1_default

# 2. mariadb
docker run -d --name mariadb --network proyecto_php_gestion_innovacion_curricular1_default --restart unless-stopped `
  -e "MARIADB_ROOT_PASSWORD=paradigmas123" `
  -e "MARIADB_DATABASE=innovacion_php_local" `
  -e "MARIADB_USER=innovacion" `
  -e "MARIADB_PASSWORD=paradigmas123" `
  -v mariadbdata:/var/lib/mysql `
  -v "${PWD}/db/init.sql:/docker-entrypoint-initdb.d/init.sql:ro" `
  -p 13332:3306 mariadb:11
...
docker run -d --name api-innovacion --network proyecto_php_gestion_innovacion_curricular1_default --restart unless-stopped `
  -e "DB_DSN=mysql:host=mariadb;port=3306;dbname=innovacion_php_local" `
  -e "DB_USUARIO=innovacion" `
  -e "DB_CLAVE=paradigmas123" `
  -v "${PWD}/api_innovacion:/app" `
  -p 8115:8115 api-innovacion

# 5. Construir la imagen de front-php y encenderla
docker build -t front-php ./front_php
docker run -d --name front-php --network proyecto_php_gestion_innovacion_curricular1_default --restart unless-stopped `
  -e "URL_API=http://api-innovacion:8115" `
  -v "${PWD}/front_php:/app" `
  -p 8114:8114 front-php

# 6. phpmyadmin
docker run -d --name phpmyadmin --network proyecto_php_gestion_innovacion_curricular1_default --restart unless-stopped `
  -e "PMA_HOST=mariadb" `
  -e "PMA_USER=innovacion" `
  -e "PMA_PASSWORD=paradigmas123" `
  -p 8107:80 phpmyadmin:latest
...
```yaml
URL_API: http://api-innovacion:8115
```
...
|---|---|---|
| **Su navegador** | `http://localhost:8115` | Está fuera de Docker. Usa el puerto **publicado** |
| **Otro contenedor** | `http://api-innovacion:8115` | Están en la misma red: se llaman por el **nombre del servicio** |
...
> **En este proyecto los dos números son iguales** (`8115:8115`), y eso
...
Si alguien cambiara esa línea por `http://localhost:8115`, el front
...
```powershell
docker network inspect proyecto_php_gestion_innovacion_curricular1_default
```
...
`docker-compose.yml` y le pega `_default`. La carpeta `proyecto_php_gestion_innovacion_curricular1` produce
la red `proyecto_php_gestion_innovacion_curricular1_default`. Tres formas de confirmarlo:
...
docker inspect proyecto_php_gestion_innovacion_curricular1-front-php-1 --format '{{range $k,$v := .NetworkSettings.Networks}}{{$k}}{{end}}'
```
...
network proyecto_php_gestion_innovacion_curricular1_default has active endpoints (name:"proyecto_php_gestion_innovacion_curricular1-front-php-1" …)
```
