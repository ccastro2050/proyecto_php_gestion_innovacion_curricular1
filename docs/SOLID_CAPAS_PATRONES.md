...
│ controladores/ControladorAliado.php               │
...
│ repositorios/RepositorioAliadoMariaDB.php         │
│ El SQL con PDO: traduce filas ↔ objetos Aliado.   │
│ NO conoce HTTP. NO decide negocio.                  │
└────────────────┬────────────────────────────────────┘
                 │  ④ SELECT … FROM aliado WHERE codigo = :codigo
                 ▼
          ┌───────────────┐
          │ BASE DE DATOS │  MariaDB — innovacion_php_local
          └───────┬───────┘
                  │
   y la respuesta hace el viaje DE VUELTA:
   fila → objeto Aliado (repositorio) → objeto (servicio)
...
|---|---|---|---|
| **Controlador** | HTTP: validar el body, códigos de estado, JSON | SQL y reglas de negocio | `controladores/ControladorAliado.php` |
| **Servicio** | Las reglas del negocio (¿existe? ¿se puede?) | Saber de HTTP o del motor de BD | `servicios/ServicioAliado.php` |
| **Repositorio** | El SQL y el mapeo fila ↔ objeto | Saber de HTTP o decidir negocio | `repositorios/RepositorioAliadoMariaDB.php` |
...
```
CAPA 1: FRONT (:8114)   → solo pinta y llama la API; nunca toca la BD
CAPA 2: API   (:8115)   → solo JSON
...
Pregunta legítima: la carpeta de modelos (`modelos/Aliado.php`) existe en el
...
- El **modelo es LO QUE VIAJA entre estaciones**: el repositorio arma un
  `Aliado` desde la fila, el servicio lo razona, el controlador lo
...
(`RepositorioAliadoPostgreSQL implements IRepositorioAliado`) y ajustar el
...
class RepositorioAliadoPostgreSQL implements IRepositorioAliado { /* … */ }

// ...y el ensamblador (ÚNICO archivo tocado) elegirá el motor:
$repositorio = $motor === 'postgres'
    ? new RepositorioAliadoPostgreSQL($dsn, $usuario, $clave)
    : new RepositorioAliadoMariaDB($dsn, $usuario, $clave);
```
...
`RepositorioAliadoMariaDB` son indistinguibles para el servicio — por eso la
...
{
    public function obtenerPorCodigo(string $codigo): ?Aliado
    {
...
$servicio = new ServicioAliado(new RepositorioFalsoEnMemoria());
...
**En la v1:** `ServicioAliado` recibe **la interfaz** por constructor
(`private readonly IRepositorioAliado $repositorio`); solo
...
class ServicioAliado implements IServicioAliado
{
    public function __construct(
        private readonly IRepositorioAliado $repositorio,  // ← interfaz, no clase
    ) {
    }
}

// El ÚNICO lugar que conoce la clase concreta (v3 lo convertirá en fábrica):
function crearServicioAliado(): IServicioAliado
{
    $repositorio = new RepositorioAliadoMariaDB(
        getenv('DB_DSN'), getenv('DB_USUARIO'), getenv('DB_CLAVE')
    );
    return new ServicioAliado($repositorio);
}
```
...
match (true) {
    $ruta === '/api/aliado' && $metodo === 'GET' => $controlador->listar(),
    // …
};
```
...
```php
// El contrato (repositorios/IRepositorioAliado.php):
public function obtenerPorCodigo(string $codigo): ?Aliado;

// ServicioAliado lo usa SIN saber si detrás hay MariaDB o un array
...
{
    public function __construct(
        private readonly IRepositorioAliado $repositorio,  // ← llega armado
    ) {
    }
}
```
...
function crearRepositorioAliado(string $motor): IRepositorioAliado
{
    return $motor === 'postgres'
        ? new RepositorioAliadoPostgreSQL($dsnPg, $usuario, $clave)
        : new RepositorioAliadoMariaDB($dsnMaria, $usuario, $clave);
}
...
(`RepositorioAliadoMariaDB`, `RepositorioFalsoEnMemoria` — y los motores
