# Plan técnico — v1: `aliado` (PHP + MariaDB)
...
├── repositorios/
│   ├── IRepositorioAliado.php
│   └── RepositorioAliadoMariaDB.php
│                           El SQL, en prepared statements. Único que conoce PDO.
├── modelos/Aliado.php     La fila como objeto: privadas + getters + setters
...
flowchart TD
    ...
    R --> C["ControladorAliado<br/>¿el cuerpo tiene la forma? si no → 422"]
    C -->|"interfaz IServicioAliado"| S["ServicioAliado<br/>las reglas"]
    S -->|"interfaz IRepositorioAliado"| Rep["RepositorioAliadoMariaDB<br/>el SQL parametrizado"]
    Rep --> BD[("MariaDB")]
...
`IServicioAliado` e `IRepositorioAliado` son interfaces nativas de PHP. **No las
...
Con `php -S 0.0.0.0:8114 index.php`, PHP ejecuta el router
