# Guía de IA — v1: `aliado` (PHP + MariaDB)
...
```powershell
# Las carpetas
mkdir docs\spec_kit\versiones\v1_aliado, db, front_php\publico, api_innovacion, api_innovacion\controladores, api_innovacion\excepciones, api_innovacion\modelos, api_innovacion\pruebas, api_innovacion\repositorios, api_innovacion\servicios, front_php, front_php\vistas, pruebas_humo

# Los archivos, vacíos: la IA los llena, uno por uno, y usted los revisa
New-Item .gitattributes, .gitignore, api_innovacion\Dockerfile, api_innovacion\controladores\ControladorAliado.php, api_innovacion\excepciones\NoEncontradoExcepcion.php, api_innovacion\index.php, api_innovacion\modelos\Aliado.php, api_innovacion\pruebas\prueba_capas.php, api_innovacion\repositorios\IRepositorioAliado.php, api_innovacion\repositorios\RepositorioAliadoMariaDB.php, api_innovacion\servicios\IServicioAliado.php, api_innovacion\servicios\ServicioAliado.php, api_innovacion\servicios\ensamblador.php, docker-compose.yml, front_php\Dockerfile, front_php\cliente_api.php, front_php\index.php, front_php\vistas\formulario.php, front_php\vistas\inicio.php, front_php\vistas\lista.php, front_php\vistas\no_encontrada.php, front_php\vistas\plantilla.php, pruebas_humo\humo_front.py
```
...
tú creas mejor:
  ...
  · docs/spec_kit/versiones/v1_aliado/2_spec.md    qué hay que construir
  · docs/spec_kit/versiones/v1_aliado/6_contracts.md   cada ruta, exacta
...
3. La tabla es `aliado`, con llave `nit` y estos campos:
   nit (entero), razon_social (texto), nombre_contacto (texto), correo (texto), telefono (texto), ciudad (texto)
...
6. La base YA VIENE DADA en db/init.sql. No la modifiques ni escribas SQL de
   creación de tablas. La tabla `aliado` arranca con 14
   filas sembradas.
...
9. LA VERSIÓN INCLUYE SU PANTALLA, y es la mitad del trabajo, no un añadido.
   ...
   publicando el puerto 8114:

   · una pantalla por recurso, con DIRECCIÓN PROPIA (/aliados),
     nunca una ruta con el nombre de la tabla como parámetro;
   · una FUNCIÓN POR OPERACIÓN —listar_innovacion, crear_innovacion…—,
   ...
   d) Y NO compartas código entre la API y el front. Las dos están en PHP y
      ...
      require_once __DIR__ . '/../api_innovacion/modelos/Aliado.php'
...
| **`activo` en el modelo o en la lista blanca** | Es una columna de la tabla, parece un campo más | ¿`Aliado::toArray()` lo devuelve? No debe |
| **Un cliente genérico en el front** | Es más corto, y con una sola tabla ni se nota | ¿Las funciones se llaman `listar_innovacion` o `listar($recurso)`? |
| **`require` del modelo de la API en el front** | Están ahí al lado y ahorra escribirlos | ¿Hay algún `require` que apunte a `../api_innovacion/`? |

## 4. Y lo que la IA no va a hacer sola

Comparar con el gemelo. Este mismo módulo está en
`proyecto_paradigmas_innovacion_curricular1`, en Python y FastAPI. Levante los dos,
