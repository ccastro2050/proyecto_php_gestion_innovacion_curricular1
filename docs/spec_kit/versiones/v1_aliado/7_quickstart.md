# Quickstart — v1: `aliado` (PHP + MariaDB)
...
> falta: `curl -s -X POST $API/api/aliado -H "Content-Type: application/json" -d '…'`
> funciona tal cual.
...
pedir GET /
#    → mensaje: API de Innovación curricular funcionando · version: v1 · tabla: aliado

# 2. El sistema arranca CON DATOS: el init.sql sembró la tabla
(pedir GET /api/aliado).total
#    → 14
#      El 204 sigue siendo la respuesta al listado VACÍO —y el front lo trata
#      como «todavía no hay»— pero no se llega ahí arrancando.

# 3. Crear y listar
pedir POST /api/aliado '{"nit": 900123456, "razon_social": "Empresa de Ejemplo", "nombre_contacto": "Ana Ruiz", "correo": "contacto@empresa.example.com", "telefono": "+57 604 555 0199", "ciudad": "Medellin"}'
#    → estado 200 · "Aliado creado exitosamente."
(pedir GET /api/aliado).total
#    → 15

# 4. El ciclo de los cinco verbos
pedir PUT /api/aliado/900123456 '{"razon_social": "Empresa de Ejemplo", "nombre_contacto": "Ana Ruiz", "correo": "contacto@empresa.example.com", "telefono": "+57 604 555 0199", "ciudad": "Medellin"}'
#    → estado 200 · filasAfectadas 1

pedir PATCH /api/aliado/900123456 '{"ciudad": "Medellin"}'
#    → estado 200 · filasAfectadas 1

pedir GET /api/aliado/900123456
#    → la ficha, con el valor nuevo

# 4b. La pareja que enseña la diferencia: MISMO cuerpo, dos verbos
pedir PUT /api/aliado/900123456 '{"nombre_contacto": "Ana Ruiz", "correo": "contacto@empresa.example.com", "telefono": "+57 604 555 0199", "ciudad": "Medellin"}'
#    → HTTP 422: al PUT le falta 'razon_social', y reemplazar exige todo

pedir PATCH /api/aliado/900123456 '{"nombre_contacto": "Ana Ruiz", "correo": "contacto@empresa.example.com", "telefono": "+57 604 555 0199", "ciudad": "Medellin"}'
#    → estado 200: al PATCH le basta con lo enviado

# 5. El borrado es LÓGICO, y se comprueba
pedir DELETE /api/aliado/900123456
#    → estado 200 · filasAfectadas 1
(pedir GET /api/aliado).total
#    → 14 otra vez
pedir DELETE /api/aliado/900123456
...
docker compose exec mariadb `
  mariadb -uinnovacion -pparadigmas123 innovacion_php_local `
  -e "SELECT nit, activo FROM aliado WHERE nit = 900123456"
#    → 900123456 | 0   ← sigue ahí, con activo en cero

# 6. La validación es la frontera: nada de esto llega a la base
pedir POST /api/aliado '{"nombre_contacto": "Ana Ruiz", "correo": "contacto@empresa.example.com", "telefono": "+57 604 555 0199", "ciudad": "Medellin"}'
#    → HTTP 422, y los errores VIENEN EN ESPAÑOL y nombran el campo:
#      "El campo nit es obligatorio…", "El campo razon_social es obligatorio."
...
docker compose exec api-innovacion php pruebas/prueba_capas.php
...
Abrir **http://localhost:8114** y:

1. entrar a **Aliados**: se ven las 14 filas;
...
| `Connection refused` en el 8115 | La API no arrancó. `docker compose logs api-innovacion` |
