...
**Éxito de lectura:**
```json
{ "tabla": "aliado", "limite": 1000, "total": 3, "datos": [ … ] }
```
...
> ([`proyecto_paradigmas_innovacion_curricular1`](../../../../../proyecto_paradigmas_innovacion_curricular1))
...
```
→ 200 { "mensaje":"API de Innovación curricular funcionando", "version":"v1",
         "tabla":"aliado", "contratos":"…" }
```

## 2. `GET /api/aliado` — listar

Parámetro opcional: `limite` (entero > 0, por defecto 1000).

```
→ 200 { "tabla":"aliado", "limite":1000, "total":14, "datos":[…] }
...
## 3. `GET /api/aliado/{nit}` — obtener una

```
→ 200 {"nit": 900123456, "razon_social": "Empresa de Ejemplo", "nombre_contacto": "Ana Ruiz", "correo": "contacto@empresa.example.com", "telefono": "+57 604 555 0199", "ciudad": "Medellin"}
→ 404 { "estado":404, "mensaje":"Aliado no encontrado.",
         "detalle":"No existe el aliado con nit = …" }
```
...
```json
{"nit": 900123456, "razon_social": "Empresa de Ejemplo", "nombre_contacto": "Ana Ruiz", "correo": "contacto@empresa.example.com", "telefono": "+57 604 555 0199", "ciudad": "Medellin"}
```
...
## 5. `PUT /api/aliado/{nit}` — reemplazar

Cuerpo: **todos** los campos **menos la llave**, que va en la ruta.

```json
{"razon_social": "Empresa de Ejemplo", "nombre_contacto": "Ana Ruiz", "correo": "contacto@empresa.example.com", "telefono": "+57 604 555 0199", "ciudad": "Medellin"}
```

```
→ 200 { "estado":200, "mensaje":"Aliado reemplazado.", "filasAfectadas":1 }
→ 422 { …, "errores":["El campo razon_social es obligatorio."] }
→ 404 { "estado":404, "mensaje":"Aliado no encontrado.", … }
```

**Este cuerpo da 422** —le falta `razon_social`— y es el mismo que el PATCH de
abajo acepta:

```json
{"nombre_contacto": "Ana Ruiz", "correo": "contacto@empresa.example.com", "telefono": "+57 604 555 0199", "ciudad": "Medellin"}
```

## 6. `PATCH /api/aliado/{nit}` — actualizar
...
## 7. `DELETE /api/aliado/{nit}` — retirar
...
| `204` | «Todavía no hay aliados» y el botón de agregar — **no es un error** |
