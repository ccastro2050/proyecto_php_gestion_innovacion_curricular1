# Especificación — v1: `aliado` (PHP + MariaDB)

## 1. Qué construye esta versión

El CRUD completo de **`aliado`**, la tabla sin clave foránea con más campos
del módulo de Innovación curricular: **API en PHP puro** y **pantalla en PHP**, cada
una en su contenedor, sobre **MariaDB**.

## 2. Lo que esta versión NO incluye

- Las otras 24 tablas: llegan en la v2 y la v3, cada una
...
| Campo | Tipo | | Nota |
|---|---|---|---|
| `nit` | entero | obligatorio | El NIT del aliado. Es la llave. |
| `razon_social` | texto, hasta 60 | obligatorio | — |
| `nombre_contacto` | texto, hasta 60 | obligatorio | — |
| `correo` | texto, hasta 70 | obligatorio | Se guarda como texto: el esquema dado no exige un formato de correo. |
| `telefono` | texto, hasta 45 | obligatorio | — |
...
### RF7 — Diagnóstico
`GET /` → 200 con `{"version":"v1", "tabla":"aliado"}`.

## 5. Criterios de aceptación

1. **Un solo comando.** `docker compose up -d --build` deja corriendo MariaDB
   —con la base y sus 25 tablas—, la API y la pantalla.
   `GET http://localhost:8115/` responde `"version":"v1"`.
2. **El sistema arranca CON DATOS.** `GET /api/aliado` responde **200** con
   `total: 14`.
3. **Crear y listar.** Un `POST` válido responde 200; después el listado
   responde `total: 15`.
4. **El ciclo de los cinco verbos.** `POST` crea `900123456` → `PUT` lo
   reemplaza → `PATCH` le cambia **un** campo → `GET` lo confirma → `DELETE`
   lo retira, y un **segundo** `DELETE` responde **404**.
   Además: un `PUT` sin `razon_social` responde **422** mientras el **mismo
...
6. **La validación es la frontera.** Un `POST` sin `razon_social` → **422** con
...
| P2 | Cada pantalla tiene **dirección propia** | `/aliados`, `/aliados/nuevo`, `/aliados/{clave}/editar` — ninguna con el nombre de la tabla como parámetro |
...
| C3 | El esquema dado no tiene columna para el borrado lógico | **Las 22 tablas del módulo ganan `activo BOOLEAN NOT NULL DEFAULT TRUE`.** En MariaDB `BOOLEAN` es alias de `TINYINT(1)`: se escribe `BOOLEAN` porque dice lo que la columna significa | `db/init.sql` [C4] |
...
| C6 | ¿La API valida que `nit` no sea una llave ajena a la tabla? | **No hay claves foráneas en esta tabla**, así que no hay integridad referencial que imponer todavía. Llega en la v3 | [4_research.md](4_research.md) |
