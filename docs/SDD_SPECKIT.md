...
> 5. **El código pasa a ser un subaliado:** si toda la lógica vive en
...
clarify NO crea un archivo: escribe dentro de 2_spec.md
    ## Clarificaciones
    C5 — DELETE, ¿físico o lógico? -> Físico: la tabel aliado no
...
4. **Es evaluable y comparable.** Los `.md` se leen y se califican, y los
   cursos comparten la misma estructura sobre la misma `innovacion_php_local` con
   stacks distintos.
...
- **La versión en curso:** [spec_kit/versiones/v1_aliado/](spec_kit/versiones/v1_aliado/2_spec.md)
...
`PATCH /api/aliado/{codigo}` con body parcial (campos opcionales):
...
```bash
curl http://localhost:8115/api/aliado        # total: 8
curl -i http://localhost:8115/api/aliado/PR999   # → 404
```
...
- [ ] modelos/Aliado.php (la entidad: 4 propiedades)
- [ ] peticiones/AliadoCrear.php (validación: todo obligatorio)
...
```markdown
### RF6 — Eliminar aliado
DELETE /api/aliado/{codigo} elimina el aliado.
...
|---|---|---|---|---|
| RF1 — listar | §2 `GET /api/aliado` | Fase 5 · controlador | §2 comando 2 | 1 y 2 |
...
7. En este repositorio: la [constitución](spec_kit/1_constitution.md), el
   ...
   [spec kit de la v1](spec_kit/versiones/v1_aliado/2_spec.md).
