...
public function testCrearAliado(): void
{
    $creado = $this->servicio->crearAliado($datos);
    // (sin assert: si crearAliado guarda mal, esta prueba PASA)
}
```
...
```php
public function testCrearAliadoGuardaElNombre(): void
{
    $creado = $this->servicio->crearAliado($datos);
