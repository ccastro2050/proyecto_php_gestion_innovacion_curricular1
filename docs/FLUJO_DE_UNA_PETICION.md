...
| **La ruta** | `$ruta = parse_url($_SERVER['REQUEST_URI'], PHP_URL_PATH);` | `$ruta` vale `"/api/aliado"` |
...
```php
if ($ruta === '/api/aliado') {          // ¿pidieron la colección?
    if ($metodo === 'GET') {
...
```
1. index.php        captura  $metodo="POST", $ruta="/api/aliado", $body=[...]
2. index.php        compara  y llama $controlador->crear($body)
3. ControladorAliado::crear
...
4. ServicioAliado::crear
      construye el objeto del modelo (new Aliado(...))
      y se lo pasa al repositorio (por la interfaz)
5. RepositorioAliadoMariaDB::crear
      INSERT INTO aliado (...) VALUES (:codigo, :nombre, ...)  ← PDO preparado
...
```mermaid
flowchart TD
    A["1. php -S / php-fpm recibe<br/>el texto HTTP"] --> B["2. index.php encuentra<br/>POST /api/aliado<br/>en la tabla de rutas"]
...
```
1. index.php        captura  $metodo="GET", $ruta="/api/aliado/PR001"
2. index.php        recorta  el código con substr → "PR001"
                    y llama  $controlador->obtener("PR001")
3. ControladorAliado::obtener   try { responder(200, servicio->obtener(...)) }
4. ServicioAliado::obtener      pide al repositorio; si llega null → lanza
                                  NoEncontradoExcepcion (el catch la vuelve 404)
5. RepositorioAliadoMariaDB     SELECT ... WHERE codigo = :codigo
6. La fila vuelve convertida en objeto Aliado y sale como JSON
```
...
Invoke-RestMethod "http://localhost:8115/api/aliado"
Invoke-RestMethod "http://localhost:8115/api/aliado/PR001"

# POST — crear
Invoke-RestMethod -Method Post -Uri "http://localhost:8115/api/aliado" -ContentType "application/json" -Body '{"codigo":"PR009","nombre":"Webcam","stock":10,"valorunitario":350000}'

# PUT con body incompleto → error 422 (PUT exige TODO)
Invoke-RestMethod -Method Put -Uri "http://localhost:8115/api/aliado/PR009" -ContentType "application/json" -Body '{"stock":25}'

# PATCH con el MISMO body → 200 (PATCH es parcial)
Invoke-RestMethod -Method Patch -Uri "http://localhost:8115/api/aliado/PR009" -ContentType "application/json" -Body '{"stock":25}'

# DELETE — limpiar
Invoke-RestMethod -Method Delete -Uri "http://localhost:8115/api/aliado/PR009"
```
