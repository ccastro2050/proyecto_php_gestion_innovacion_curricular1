 ...
 * memoria que implementa IRepositorioAliado, **sin MariaDB corriendo**.
 *
...
require_once __DIR__ . '/../servicios/ServicioAliado.php';
...
class RepositorioFalsoEnMemoria implements IRepositorioAliado
{
    ...
    public function obtenerPorClave(int $clave): ?Aliado
    {
    ...
    public function actualizar(int $clave, array $datos): int
    {
        ...
        if (array_key_exists('nombre_contacto', $datos)) {
            $fila->setNombreContacto($datos['nombre_contacto']);
        }
        if (array_key_exists('correo', $datos)) {
            $fila->setCorreo($datos['correo']);
        }
        if (array_key_exists('telefono', $datos)) {
            $fila->setTelefono($datos['telefono']);
        }
...
// ----------------------------------------------------------------------
// La prueba: el MISMO ServicioAliado, con otro repositorio (polimorfismo)
// ----------------------------------------------------------------------
...
$servicio->crear([
            ...
            'correo' => 'contacto@empresa.example.com',
            'telefono' => '+57 604 555 0199',
            'ciudad' => 'Medellin',
]);
verificar($servicio->listar(10)[0]->getNit() === $clave, 'crear y listar');
verificar($servicio->obtener($clave) instanceof Aliado,           'obtener por llave');

verificar($servicio->actualizar($clave, ['razon_social' => 'Cambiado']) === 1,
          'actualizar un solo campo');
verificar($servicio->obtener($clave)->getRazonSocial() === 'Cambiado',
