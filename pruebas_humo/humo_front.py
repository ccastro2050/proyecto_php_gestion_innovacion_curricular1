# -*- coding: utf-8 -*-
"""Prueba de humo del FRONT de Innovación curricular (PHP, puerto 8114).
...
# ======================================================================
PANTALLAS = [
    ("/", "Sistema de innovación curricular"),
    ("/aliados", "Aliados"),
...
revisar("  y el motivo está EN ESPAÑOL y nombra el campo",
        "razon_social" in texto and "should be" not in texto)
...
c, t = enviar("/aliados/" + str(LLAVE) + "/editar",
              dict(vacios, verbo="parcial",
                   **{"nombre_contacto": "Solo esto " + SUFIJO}))
...
revisar("  el campo enviado cambió",
        ficha and ficha.get("nombre_contacto") == "Solo esto " + SUFIJO,
...
# --- Retirar ---
c, t = enviar("/aliados/" + str(LLAVE) + "/retirar", {})
...
revisar("  la API tampoco la entrega: para quien la usa, ya no está",
        api("/api/aliado/" + str(LLAVE)) is None)
...
revisar("  con su menú y su marco intactos",
        "Inicio" in texto and "Innovación curricular" in texto)
...
revisar("  y SIN un solo dato: el front no puede llegar a la base solo",
        not any(str(f["nit"]) in texto for f in filas[:3]) if filas else True)
